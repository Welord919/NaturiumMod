...
namespace NaturiumMod.Content.Items.Accessories.CraftingTrees
{
    ...
    {
        public override string Texture => "NaturiumMod/Assets/Items/Accessories/GreedyVenomFusionDragonRing";
        ...
        public override void RightClick(Player player)
        {
            ...
            {
                GVenomMode.Melee => GVenomMode.Ranged,
                GVenomMode.Ranged => GVenomMode.Magic,
                GVenomMode.Magic => GVenomMode.Summoner,
                GVenomMode.Summoner => GVenomMode.Melee,
                _ => GVenomMode.Melee
            };
    ...
    {
        public bool active;
        public GVenomMode mode = GVenomMode.Melee;
    ...
    public class GreedyVenomFusionDragonGlobalItem : GlobalItem
    {
        ...
        public override void OnHitNPC(Item item, Player player, NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            // MELEE MODE
            if (mp.mode == GVenomMode.Melee && item.DamageType == DamageClass.Melee)
                GreedyVenomHelper.ApplyVenom(target);

            // RANGED MODE
            if (mp.mode == GVenomMode.Ranged && item.DamageType == DamageClass.Ranged)
                GreedyVenomHelper.ApplyVenom(target);

            // MAGIC MODE
            if (mp.mode == GVenomMode.Magic && item.DamageType == DamageClass.Magic)
                GreedyVenomHelper.ApplyVenom(target);

            // SUMMONER MODE (whips)
            if (mp.mode == GVenomMode.Summoner && item.DamageType == DamageClass.SummonMeleeSpeed)
                GreedyVenomHelper.ApplyVenom(target);
        }
    ...
    public class GreedyVenomFusionDragonGlobalProj : GlobalProjectile
    {
        ...
        public override void OnHitNPC(Projectile proj, NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (mp.mode == GVenomMode.Melee && proj.DamageType == DamageClass.Melee)
                GreedyVenomHelper.ApplyVenom(target);

            // RANGED MODE (arrows, bullets, thrown)
            if (mp.mode == GVenomMode.Ranged && proj.DamageType == DamageClass.Ranged)
                GreedyVenomHelper.ApplyVenom(target);

            // MAGIC MODE
            if (mp.mode == GVenomMode.Magic && proj.DamageType == DamageClass.Magic)
                GreedyVenomHelper.ApplyVenom(target);

            // SUMMONER MODE (minions + whips)
            if (mp.mode == GVenomMode.Summoner && proj.DamageType == DamageClass.Summon)
                GreedyVenomHelper.ApplyVenom(target);
        }
    }
}
