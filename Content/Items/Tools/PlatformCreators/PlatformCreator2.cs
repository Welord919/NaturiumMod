...
public class PlatformCreator2 : ModItem
{
    ...
    public override bool? UseItem(Player player)
    {
        ...
        {
            int radius = count / 2; // radius used for SendTileSquare; covers a square of (2*radius+1) tiles
    ...
    {
        const int platformCount = 25;
        const int barCount = 10;
        int craftingTile = TileID.Anvils;

        // Recipe for Crimtane worlds
        Recipe recipe = CreateRecipe();
        recipe.AddIngredient(ItemID.WoodPlatform, platformCount);
        recipe.AddIngredient(ItemID.CrimtaneBar, barCount);
        ...
        recipe = CreateRecipe();
        recipe.AddIngredient(ItemID.WoodPlatform, platformCount);
        recipe.AddIngredient(ItemID.DemoniteBar, barCount);
        recipe.AddTile(craftingTile);
        recipe.Register();
    }
}