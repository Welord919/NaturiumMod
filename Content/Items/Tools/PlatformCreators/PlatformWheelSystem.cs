    ...
    public void PlacePlatforms(int count, bool replace)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (replace)
            {
                ...
                if (WorldGen.PlaceTile(x, y, platformTileType, mute: true, forced: true, plr: -1, style: 0))
                    placedAny = true;
            }
            else
            {
                if (WorldGen.PlaceTile(x, y, platformTileType, mute: true, forced: false, plr: -1, style: 0))
                    placedAny = true;
            }
        }
